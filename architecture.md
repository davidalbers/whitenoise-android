...
`MainActivity` starts and binds to `AudioPlayerService`, but only to call `handoffControl()`/`dismissNotification()`, which control the Service's own notification. `AudioController` is provided as an app-scoped `@Singleton` and injected directly into both `AudioPlayerService` and `PlayerScreenViewModel` so that both can control audio.
